...
func 创建可选选项_数据处理 ():
	...
	for 循环次数 in 列表选择框的数据.size () :
		...
		if UI类型_string == "List" || UI类型_string == "SelectBox" :
			var 索引 = 当前行数据_string.substr (当前行数据_string.find (">") + 1)
			索引 = int (索引)
			if 索引 == 索引列表选中选项_int :
				if 可生成的选项数 >= 从第N行开始显示 :
					if 最大项目数 != -1 && 可生成的选项数 - 从第N行开始显示 >= 最大项目数 :
						总共要显示的项目数 = 总共要显示的项目数 + 1
					else :
	...
	if 要生成的选项数据_list.size () != 0 :
		...
		for 生成的第n个选项 in 要生成的选项数据_list.size () :
			...
			if 要生成的选项数据_list.size () == 1:
				...
				elif 生成的第n个选项 == 要生成的选项数据_list.size () - 1:
...
func 刷新列表_显示 ():
	...
	if 要生成的选项数据_list.size() != 0 :
		...
		所有选项物体_node[0].call_deferred ("模拟左键点击")
#————————————————————————————————#————————————————————————————————#—————————————#
